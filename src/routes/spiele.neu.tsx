...
export const Route = createFileRoute("/spiele/neu")({
  component: NewSpiel,
  head: () => ({ meta: [{ title: "Neues Spiel — Spiele" }] }),
});
...
function NewSpiel() {
  ...
  const [subcategory, setSubcategory] = useState<string>("Fangspiele");
  ...
  const previewExercise: Exercise = useMemo(() => ({
    id: "preview",
    title: title.trim() || "Titel des Spiels",
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3 px-4 py-4">
          <Link to="/spiele" className="inline-flex items-center gap-2 rounded-md px-2 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-100">
      ...
      <main className="mx-auto max-w-3xl px-4 py-6">
        ...
        <form onSubmit={submit} className="space-y-6">
          ...
          <div className="sticky bottom-0 -mx-4 flex items-center justify-end gap-2 border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur">
            ...
            <Link to="/spiele" className="rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100">
              Abbrechen
            </Link>
